...
| **Pocket-Dongle-S3** ![screenshot](./docs/images/pocketdongle.png) | Very similar looking device to the LilyGo T-Dongle S3 but with a different SD card and button form factor. The device doesn't have an LED or QWIIC interface slot. Internally this device uses the older SD card interface so expect slow transfer speeds. | <ul><li>[AliExpress](https://s.click.aliexpress.com/e/_EIZx5Sw)</li></ul>
